...
//   X,
// } from "lucide-react";
// import { getReviewDistribution, reviews } from "@/data/products";
...
//               className="absolute inset-0 rounded-full border border-primary/20 border-t-primary"
//             />
//           </div>
//           <p className="font-ui text-[10px] tracking-[0.3em] text-white/30 uppercase">
//             Loading product…
//           </p>
//         </div>
//       </div>
...
//       ? product.image
//       : productImages[product.id] || "/placeholder.svg";
//   const discount = product.originalPrice
//     ? Math.round(
//         ((product.originalPrice - product.price) / product.originalPrice) * 100,
//       )
...
import { getReviewDistribution, reviews as staticReviews } from "@/data/products";
import { productImages } from "@/assets/imageMap";
    ...
    <div>
      ...
      {logoFile ? (
        ...
        <motion.div
          ...
          onClick={() => inputRef.current?.click()}
          animate={{
            borderColor: dragging ? "rgba(212,175,55,0.5)" : "rgba(212,175,55,0.2)",
          }}
          className={`border border-dashed rounded-xl p-6 text-center cursor-pointer transition-colors duration-200 ${
            dragging ? "bg-primary/8" : "bg-white/2 hover:bg-white/4"
          }`}
...
function ReviewSkeleton() {
  return (
    <div className="bg-white/[0.025] border border-white/7 rounded-2xl p-6 animate-pulse">
      ...
      <div className="flex gap-0.5 mb-3">
        {[...Array(5)].map((_, i) => (
          <div key={i} className="w-3 h-3 rounded-sm bg-white/8" />
        ))}
      </div>
      <div className="space-y-2">
        <div className="h-3 w-3/5 bg-white/8 rounded-full" />
...
export default function ProductDetailPage() {
  ...
  // ── Fetch Sanity reviews, fall back to static ──
  useEffect(() => {
    if (!id) return;
    let cancelled = false;
    ...
    getReviews(id)
      .then((data: Review[]) => {
        ...
        if (data && data.length > 0) {
          ...
          setReviews(staticReviews.filter((r) => r.productId === id) as Review[]);
        }
  ...
  if (!product) {
    return (
      <div className="min-h-screen bg-[#0a0a0a] pt-32 text-center px-4">
        ...
        <Link to="/shop" className="text-primary font-ui text-sm tracking-wider">
          ← Back to Shop
        </Link>
      </div>
    );
  ...
  // ✅ Sanity CDN URL first, local map fallback
  const image =
    product.image && product.image.startsWith("http")
      ? product.image
      : productImages[product.id] || "/placeholder.svg";
  ...
  const handleWhatsApp = () => {
    const customization = [addLogo ? "Logo" : "", message || "", giftWrap ? "Gift wrap" : ""]
      .filter(Boolean).join(", ");
    openWhatsApp(generateProductWhatsAppMessage({
      name: product.name,
      price: product.price,
      quantity,
      customization: customization || "None",
  ...
  const accordionItems = [
    ...
    },
    {
      key: "customization",
      title: "Customization",
      content: "Logo engraving, embossing, screen printing, and laser etching available. Mockups within 24 hours. Min 10 units for custom branding.",
    },
    {
      key: "shipping",
      title: "Shipping & Delivery",
      content: "Standard: 3–5 days (Free over PKR 5,000)\nExpress: 1–2 days (PKR 800)\nSame-Day: Karachi only (PKR 1,500)\nInternational: 7–14 days",
    },
    {
      key: "returns",
      title: "Returns & Guarantee",
      content: "30-day returns on unused items. Full refund or exchange. Customized items are non-returnable. Quality guaranteed.",
    },
  ];

  // Review distribution from live reviews or static helper
  const reviewDist = (() => {
    if (reviews.length === 0) return getReviewDistribution(product.id);
    const dist: Record<number, number> = { 5: 0, 4: 0, 3: 0, 2: 0, 1: 0 };
    reviews.forEach((r) => { dist[r.rating] = (dist[r.rating] || 0) + 1; });
    const total = reviews.length;
    const pct: Record<number, number> = {};
    for (const star of [5, 4, 3, 2, 1]) {
      pct[star] = Math.round((dist[star] / total) * 100);
    }
    return pct;
  })();
      ...
      <div className="container mx-auto px-4 lg:px-8 py-8 pt-28">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 lg:gap-14">
          ...
          >
            ...
            <div className="text-sm">
              {product.inStock ? (
                product.stockCount && product.stockCount < 10 ? (
                  <p className="text-amber-400 font-medium">Only {product.stockCount} left in stock</p>
                ) : (
                  <p className="text-emerald-400">In Stock — Ships within 1–2 days</p>
                )
              ) : (
                <p className="text-red-400">Out of Stock</p>
              )}
            ...
            <p className="text-sm text-white/45 leading-relaxed">{product.shortDescription}</p>
            ...
            {/* Bulk pricing */}
            <div className="bg-white/[0.025] border border-white/7 rounded-xl p-4">
              <p className="font-ui text-[8.5px] tracking-[0.3em] text-primary mb-3 uppercase">Bulk Pricing</p>
              <div className="grid grid-cols-3 gap-x-4 gap-y-1.5 text-xs">
                <span className="text-white/25">Qty</span>
                <span className="text-white/25">Price/ea</span>
                <span className="text-white/25">Savings</span>
                <span className="text-white/70">1–9</span>
                <span className="text-white/70">{formatPrice(product.price)}</span>
                <span className="text-white/30">—</span>
                <span className="text-white/70">10–49</span>
                <span className="text-white/70">{formatPrice(Math.round(product.price * 0.89))}</span>
                <span className="text-emerald-400">11%</span>
                <span className="text-white/70">50–99</span>
                <span className="text-white/70">{formatPrice(Math.round(product.price * 0.78))}</span>
                <span className="text-emerald-400">22%</span>
                <span className="text-white/70">100+</span>
                <span className="text-primary">Contact</span>
                <span className="text-emerald-400">30%+</span>
            ...
            <div className="space-y-3">
              <label className="flex items-center gap-3 cursor-pointer">
                <div
                  ...
                  className={`w-11 h-6 rounded-full relative transition-colors duration-300 cursor-pointer ${addLogo ? "gradient-gold" : "bg-white/10"}`}
                >
                  <div className={`absolute top-0.5 w-5 h-5 rounded-full bg-white transition-transform duration-300 ${addLogo ? "translate-x-5" : "translate-x-0.5"}`} />
                </div>
                <span className="text-sm text-white/60">Add Company Logo</span>
              </label>

              {addLogo && <LogoUpload logoFile={logoFile} setLogoFile={setLogoFile} />}
              ...
              <label className="flex items-center gap-3 cursor-pointer">
                <div
                  ...
                  className={`w-11 h-6 rounded-full relative transition-colors duration-300 cursor-pointer ${message ? "gradient-gold" : "bg-white/10"}`}
                >
                  <div className={`absolute top-0.5 w-5 h-5 rounded-full bg-white transition-transform duration-300 ${message ? "translate-x-5" : "translate-x-0.5"}`} />
                </div>
                <span className="text-sm text-white/60">Personalized Message</span>
              </label>
              ...
              <label className="flex items-center gap-3 cursor-pointer">
                <div
                  ...
                  className={`w-5 h-5 rounded border-2 flex items-center justify-center transition-all ${giftWrap ? "border-primary bg-primary" : "border-white/20"}`}
                >
                  {giftWrap && <Check className="w-3 h-3 text-primary-foreground" />}
                </div>
                <span className="text-sm text-white/60">
                  Premium Gift Wrapping <span className="text-primary">+PKR 500</span>
                </span>
              </label>
            </div>
            ...
            <div className="flex items-center gap-4 flex-wrap">
              ...
              <p className="text-xs text-white/25">
                Need 50+?{" "}
                <Link to="/contact" className="text-primary cursor-pointer hover:underline">
                  Request quote
                </Link>
              </p>
            </div>
            ...
            <div className="space-y-2.5">
              <motion.button
                ...
                className="w-full h-14 rounded-xl gradient-gold font-ui text-[11px] tracking-[0.2em] text-primary-foreground flex items-center justify-center gap-2 hover:shadow-gold transition-all duration-300"
              >
                <ShoppingBag className="w-4 h-4" /> ADD TO CART — {formatPrice(totalPrice)}
              </motion.button>
              <motion.button
                onClick={handleWhatsApp}
                whileHover={{ scale: 1.01 }}
            ...
            <div className="flex flex-wrap gap-4">
              {[
                ...
                <div key={label} className="flex items-center gap-1.5">
                  <Icon className="w-3.5 h-3.5 text-white/20" />
                  <span className="text-[10px] text-white/25 font-ui tracking-wider">{label}</span>
                </div>
              ))}
            </div>
          </motion.div>
          ...
          {accordionItems.map((item) => (
            <div key={item.key} className="border-b border-white/8">
              ...
              >
                ...
                <motion.div animate={{ rotate: openAccordion === item.key ? 180 : 0 }} transition={{ duration: 0.25 }}>
                  <ChevronDown className="w-4 h-4 text-primary" />
                </motion.div>
              </button>
              <AnimatePresence>
        ...
        <div className="mt-16">
          <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-6 mb-8">
            ...
            <div className="flex-shrink-0 w-48 md:w-56 space-y-1.5">
              {[5, 4, 3, 2, 1].map((stars) => {
                const pct = reviewDist[stars] ?? 0;
                return (
                  <div key={stars} className="flex items-center gap-2">
                    <span className="text-[10px] text-white/35 w-3 font-ui flex-shrink-0">{stars}</span>
                    <div className="flex-1 h-1.5 rounded-full bg-white/6 overflow-hidden">
                      <motion.div
                        className="h-full rounded-full gradient-gold"
                        initial={{ width: 0 }}
          ...
          <div className="space-y-3">
            {reviewsLoading ? (
              ...
              <div className="py-10 text-center border border-white/7 rounded-2xl bg-white/[0.02]">
                <p className="text-sm text-white/25 font-ui tracking-wider">
              ...
              reviews.slice(0, 4).map((review) => (
                ...
                >
                  <div className="flex items-center justify-between mb-3">
                    <div className="flex items-center gap-3">
                      <div className="w-9 h-9 rounded-full gradient-gold flex items-center justify-center font-bold text-xs text-primary-foreground flex-shrink-0">
                        {review.name.charAt(0)}
                      </div>
                      <div>
                    ...
                    <div className="flex flex-col items-end gap-1">
                      <span className="text-[10.5px] text-white/30 font-ui">{review.date}</span>
                      {review.verified && (
                        <span className="flex items-center gap-1 text-[9px] text-emerald-400 font-ui tracking-wider">
                  ...
                  <div className="flex gap-0.5 mb-2">
                    {[...Array(5)].map((_, i) => (
                      <Star
                        key={i}
                        className={`w-3 h-3 ${i < review.rating ? "text-primary fill-primary" : "text-white/15"}`}
                      />
                    ))}
                  </div>
                  <p className="text-sm font-medium text-white/70 mb-1">{review.title}</p>
                  <p className="text-sm text-white/40 leading-relaxed">{review.text}</p>
                </motion.div>
              ))
            )}
          </div>
